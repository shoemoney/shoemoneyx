...
- [x] Chart page without the TradingView library (every AMI/marketplace desk, since the library is licensed and gitignored) showed "Some market information is unavailable. The chart could not load. Refresh this page to try again." A buyer will refresh forever. Fixed 2026-09-14: Chart.vue detects `!window.TradingView` and shows an "Interactive chart not installed" notice instead — what is missing, why (the license forbids redistribution), and where to drop it (public/charting_library/README.md). Genuine load failures still get the generic error banner.
- [ ] Marketplace listing. **2026-09-13:** seller registration done as an individual (Partner Central identity + business verification, public profile "ShoeMoney", W-9 complete, vendor `seller-bvpfybewuhlce`). Waiting on AWS's public-profile review email; until then the USD disbursement method (ACH *396, Daily, form already filled) cannot be added and product creation is blocked. Then: add disbursement → create AmiProduct → `MARKETPLACE_PRODUCT_ID` into `ops/image/marketplace.env` → `release.sh 0.1.2` with Intent VALIDATE (OperatingSystemName=UBUNTU is unverified against AWS's enum).
