...
- [ ] Chart page without the TradingView library (every AMI/marketplace desk, since the library is licensed and gitignored) shows "Some market information is unavailable. The chart could not load. Refresh this page to try again." A buyer will refresh forever. Detect `!window.TradingView` in resources/js/pages/Chart.vue and say what is actually missing and where to drop it (public/charting_library/README.md). Seen on dev.shoemoneyx.com 2026-09-13; the market feed, candles and indicators on that page all work.
