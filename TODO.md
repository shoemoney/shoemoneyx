...
- [ ] Chart page behind a master password: `UDFCompatibleDatafeed('/api/udf')` (resources/js/pages/Chart.vue:55) is TradingView's own fetcher and cannot send `X-Desk-Token`, while `/api/udf/*` sits inside the DeskToken group (routes/api.php:33-145), so config/history/symbols 401 once a password exists. Found by tests/e2e/login-and-ai.mjs on 2026-09-13. Needs a decision: exempt the read-only UDF routes from DeskToken, or replace the stock datafeed with an adapter that appends `?token=`.
